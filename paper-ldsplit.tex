...
{\color{red}Dynamic programming has been successfully used before to solve related problems such as haplotype block partitioning \cite[]{zhang2002dynamic}.}
...
Then, the user can choose the desired number of blocks, {\color{red}which is a compromise between having more (smaller) blocks with a higher overall cost (LD between blocks), and having less (larger) blocks with a smaller cost.}
...
{\color{red}Both methods largely pick block boundaries at recombination hotspots (Figures \ref{fig:bounds-chr22} and \ref{fig:bounds-chr1}).
...
{\color{red}The HapMap3 variants annotated with 242 blocks can be downloaded at \url{https://www.dropbox.com/s/hdui60p9ohyhvv5/map_blocks.rds?dl=1}.
...
We thank Bjarni J.\ Vilhj\'almsson for his feedback on the manuscript.
